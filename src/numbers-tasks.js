...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  return Math.acos(
    ((x2 * x1 + y2 * y1) / Math.sqrt(x1 * x1 + y1 * y1)) *
...
function toNumber(value, def) {
  return Number.isFinite(Number(value)) ? Number(value).valueOf() : def;
}
...
function getSumOfDigits(num) {
  return Array.from(num.toString()).reduce((sum, item) => sum + +item, 0);
}
